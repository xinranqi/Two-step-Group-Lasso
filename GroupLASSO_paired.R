...
paired = irgs_indx[which(irgs_indx$data_pair=="paired"),-c(1,2)]
pairedcov = irgs_cov[which(irgs_cov$data_pair=="paired"),-c(1,2)]
pairedFullInfo = aims_irgs_jan2018[which(aims_irgs_jan2018$data_pair=="paired"),]
...
Exclude = function(DATASET,cutoff,seq) {
  Nobs = OutcObsN[seq]
  DATASET = DATASET[which(DATASET$Frequency<Nobs-cutoff),1]
  DATASET
}
...
GroupLASSOCox = function(XCovariates,TIMES,EVENTS,PENALTY,GROUP, SubGroupLASSO,MinorIRGs) {
  ...
  if(SubGroupLASSO==TRUE) {
    ...
    for (i in 1:length(MinorIRGinAllIndex)) {
      ...
      if (incompleteIndx > 925) {
        completeDonor = c(completeDonor, IRGmodulusIndex)
      }
      
      if (incompleteIndx < 926 & incompleteIndx > 555) {
        completePatient = c(completePatient, IRGmodulusIndex)
      }
      
      if (incompleteIndx < 556) {
        completeMismatch = c(completeMismatch, IRGmodulusIndex)
      }
      
      completeMismatch = unique(sort(completeMismatch, decreasing=FALSE))
      completePatient = unique(sort(completePatient, decreasing=FALSE))
      completeDonor = unique(sort(completeDonor, decreasing=FALSE))
    }
    
    # complete IRG SNPs indexes
    CompleteIRGIndexs =  sum(GROUP <= sum(1-PENALTY)) + c(completeMismatch, (185 + completeMismatch), (185*2 + completeMismatch),
                                                          (185*3 + completePatient), (185*4 + completePatient),
                                                          (185*5 + completeDonor), (185*6 + completeDonor))
    
    # IRG SNPs prepared to be combined
    MinorIRGforMismatch = XCovariates[ , sum(GROUP <= sum(1-PENALTY)) + completeMismatch] + 
      XCovariates[ , sum(GROUP <= sum(1-PENALTY)) + 185 + completeMismatch] + 
      XCovariates[ , sum(GROUP <= sum(1-PENALTY)) + 185*2 + completeMismatch]
    MinorIRGforPatient = XCovariates[ , sum(GROUP <= sum(1-PENALTY)) + 185*3 + completePatient] + 
      XCovariates[ , sum(GROUP <= sum(1-PENALTY)) + 185*4 + completePatient]
    MinorIRGforDonor = XCovariates[ , sum(GROUP <= sum(1-PENALTY)) + 185*5 + completeDonor] + 
      XCovariates[ , sum(GROUP <= sum(1-PENALTY)) + 185*6 + completeDonor]
    CombinedMinorIRG = cbind.data.frame(MinorIRGforMismatch, MinorIRGforPatient, MinorIRGforDonor)
    colnames(CombinedMinorIRG) = str_sub(colnames(CombinedMinorIRG), start = -7)
...
COVARIATE1 = cbind.data.frame(
  ...
  pairedcov$firsttx1,Wmatrx_paired)
group1 = c(rep(1, times=5), rep(2, times=4), rep(3, times=6), rep(4, times=2),
           rep(5, times=4), rep(6, times=3), rep(7, times=4), 8,
           rep(seq(from=9, to=(8+185), by=1), times=3),
           rep(seq(from=(9+185), to=(8+185+185), by=1), times=2),
           rep(seq(from=(8+185+185+1), to=(8+185+185+185), by=1), times=2) )
penaltyfactor1 = c(rep(0, times=8), rep(1, times=(185*3)))
...
group2 = c(rep(1, times=5), rep(2, times=4), rep(3, times=2), rep(4, times=4),
           ...
           rep(seq(from=11, to=(10+185), by=1), times=3),
           rep(seq(from=(11+185), to=(10+185+185), by=1), times=2),
           rep(seq(from=(10+185+185+1), to=(10+185+185+185), by=1), times=2) )
penaltyfactor2 = c(rep(0, times=10), rep(1, times=(185*3)))
...
COVARIATE3 = cbind.data.frame(
  ...
  pairedcov$gvhdprop21,pairedcov$gvhdprop22,pairedcov$gvhdprop23,pairedcov$gvhdprop24,
  pairedcov$gvhdprop27,pairedcov$gvhdprop28,
  pairedcov$Dcluster1,pairedcov$Dcluster2,pairedcov$Dcluster3,pairedcov$Dcluster4,
  pairedcov$Dcluster5,pairedcov$Dcluster6,pairedcov$Dcluster7,
  Wmatrx_paired)
group3 = c(rep(1, times=5), rep(2, times=4), rep(3, times=3), rep(4, times=2),
           rep(5, times=4), rep(6, times=2), rep(7, times=4), rep(8, times=6),
           rep(9, times=7),
           rep(seq(from=10, to=(9+185), by=1), times=3),
           rep(seq(from=(10+185), to=(9+185+185), by=1), times=2),
           rep(seq(from=(9+185+185+1), to=(9+185+185+185), by=1), times=2) )
penaltyfactor3 = c(rep(0, times=9), rep(1, times=(185*3)))
...
group4 = c(rep(1, times=4), rep(2, times=6), rep(3, times=3), rep(4, times=2),
           rep(seq(from=5, to=(4+185), by=1), times=3),
           rep(seq(from=(5+185), to=(4+185+185), by=1), times=2),
           rep(seq(from=(4+185+185+1), to=(4+185+185+185), by=1), times=2) )
penaltyfactor4 = c(rep(0, times=4), rep(1, times=(185*3)))
...
group5 = c(rep(1, times=5), rep(2, times=4), rep(3, times=2), rep(4, times=2),
           ...
           rep(seq(from=(9+185), to=(8+185+185), by=1), times=2),
           rep(seq(from=(8+185+185+1), to=(8+185+185+185), by=1), times=2) )
penaltyfactor5 = c(rep(0, times=8), rep(1, times=(185*3)))
...
group6 = c(rep(1, times=5), rep(2, times=2), rep(3, times=4), rep(4, times=2),
           ...
           rep(seq(from=(10+185), to=(9+185+185), by=1), times=2),
           rep(seq(from=(9+185+185+1), to=(9+185+185+185), by=1), times=2) )
penaltyfactor6 = c(rep(0, times=9), rep(1, times=(185*3)))
...
group7 = c(rep(1, times=4), rep(2, times=6), rep(3, times=2), 4,
           ...
           rep(seq(from=7, to=(6+185), by=1), times=3),
           rep(seq(from=(7+185), to=(6+185+185), by=1), times=2),
           rep(seq(from=(6+185+185+1), to=(6+185+185+185), by=1), times=2) )
penaltyfactor7 = c(rep(0, times=6), rep(1, times=(185*3)))
...
group8 = c(rep(1, times=4), rep(2, times=6), rep(3, times=2), 4,
           ...
           rep(seq(from=(10+185), to=(9+185+185), by=1), times=2),
           rep(seq(from=(9+185+185+1), to=(9+185+185+185), by=1), times=2) )
penaltyfactor8 = c(rep(0, times=9), rep(1, times=(185*3)))
...
group9 = c(rep(1, times=5), rep(2, times=4), rep(3, times=6), rep(4, times=4),
           ...
           rep(9, times=2), rep(10, times=4), rep(11, times=4), rep(12, times=7),
           rep(seq(from=13, to=(12+185), by=1), times=3),
           rep(seq(from=(13+185), to=(12+185+185), by=1), times=2),
           rep(seq(from=(12+185+185+1), to=(12+185+185+185), by=1), times=2) )
penaltyfactor9 = c(rep(0, times=12), rep(1, times=(185*3)))
